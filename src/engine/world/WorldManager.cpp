...
size_t Engine::WorldManager::RetrieveAllGameObjects(std::vector<GameObject*>& out_GameObjects)
{
	size_t count = 0;
...
bool Engine::WorldManager::RetrieveNearestGameObject(f2 position, GameObject*& out_GameObject)
{
	...
	for (auto gameObject : m_GameObjects)
	{
		float distance = gameObject.second->m_Transform.GetTranslation().xy().GetDistanceTo(position);
...
bool Engine::WorldManager::RetrieveNearestGameObject(f3 position, GameObject*& out_GameObject)
{
	// Linear search nearest neighbor
	// TODO: this is slow, accelerate this using a dedicated data structure
	// http://en.wikipedia.org/wiki/Nearest_neighbor_search
	...
	for (auto gameObject : m_GameObjects)
	{
		float distance = gameObject.second->m_Transform.GetTranslation().GetDistanceTo(position);
		if (distance < smallestDistance)
		{
			smallestDistance = distance;
			closestGameObject = gameObject.second;
...
size_t Engine::WorldManager::RetrieveKNearestGameObjects(f2 position, size_t k, std::vector<GameObject*>& out_GameObjects)
{
	// Linear search nearest neighbor
	// TODO: this is slow, accelerate this using a dedicated data structure
	...
	for (auto gameObject : m_GameObjects)
	{
		...
		gameObjectDistanceNew.m_Distance = gameObject.second->m_Transform.GetTranslation().xy().GetDistanceTo(position);
...
size_t Engine::WorldManager::RetrieveKNearestGameObjects(f3 position, size_t k, std::vector<GameObject*>& out_GameObjects)
{
	// Linear search nearest neighbor
	// TODO: this is slow, accelerate this using a dedicated data structure
	// TODO: replace naive kNN implementation (better to quicksort and pick top k?)
	...
	for (auto gameObject : m_GameObjects)
	{
		...
		GameObjectDistance gameObjectDistanceNew;
		gameObjectDistanceNew.m_GameObject = gameObject.second;
		gameObjectDistanceNew.m_Distance = gameObject.second->m_Transform.GetTranslation().GetDistanceTo(position);
...
size_t Engine::WorldManager::RetrieveGameObjectsNearPosition(f2 position, float maxDistance, std::vector<GameObject*>& out_GameObjects)
{
	// TODO: this is slow, accelerate this using a dedicated data structure
	size_t count = 0;
	for (auto gameObject : m_GameObjects)
	{
		if (gameObject.second->m_Transform.GetTranslation().xy().GetDistanceTo(position) <= maxDistance)
		{
			out_GameObjects.push_back(gameObject.second);
			count++;
		}
...
size_t Engine::WorldManager::RetrieveGameObjectsNearPosition(f3 position, float maxDistance, std::vector<GameObject*>& out_GameObjects)
{
	// TODO: this is slow, accelerate this using a dedicated data structure
	size_t count = 0;
	for (auto gameObject : m_GameObjects)
	{
		...
		float distance = gameObject.second->m_Transform.GetTranslation().GetDistanceTo(position);
		// TESTING
		if (gameObject.second->m_Transform.GetTranslation().GetDistanceTo(position) <= maxDistance)
		{
			out_GameObjects.push_back(gameObject.second);
			count++;
		}
...
size_t Engine::WorldManager::RetrieveGameObjectsInAABB(aabb2Df aabb, std::vector<GameObject*>& out_GameObjects)
{
	// TODO: this is slow, accelerate this using a dedicated data structure
	size_t count = 0;
	for (auto gameObject : m_GameObjects)
	{
		if (aabb.Contains(gameObject.second->m_Transform.GetTranslation().xy()))
		{
			out_GameObjects.push_back(gameObject.second);
			count++;
		}
...
size_t Engine::WorldManager::RetrieveGameObjectsInAABB(aabb3Df aabb, std::vector<GameObject*>& out_GameObjects)
{
	// TODO: this is slow, accelerate this using a dedicated data structure
	size_t count = 0;
	for (auto gameObject : m_GameObjects)
	{
		if (aabb.Contains(gameObject.second->m_Transform.GetTranslation()))
		{
			out_GameObjects.push_back(gameObject.second);
			count++;
		}
